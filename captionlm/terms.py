...
def build_context_graph(
    ...
    for term in terms:
        ...
        # Shortest first: a variant can share a token prefix with another
        # (e.g. a term ending in a capital letter vs. its lowercase form can
        # tokenize identically up to a point). add_to_graph reuses an
        # existing node for a shared prefix without touching its is_end
        # flag, so if the longer variant were inserted first it would leave
        # that shared node is_end=False and the shorter form would silently
        # stop matching. A set has no defined order, so sort explicitly
